...
init : () -> ( Model, Cmd Msg )
init _ =
    ( initModel, Random.generate Fruit_Kind Fruit.fruit_kindGen )


initModel : Model
initModel =
    let snakee = Snake.Snake [ ( 1, 0 ), ( 0, 0 ) ] Right Alive 0 5 1
        fruitt=Fruit.Fruit (1,1) ""
    in
        Model  snakee fruitt 0
...
updateFruit msg ( model, cmd ) =
    case msg of
        Tick elapsed ->
            ...
            in
                ( { model | fruit = nfruit }, Cmd.batch [ cmd, Random.generate Place_Fruit fruitGen, Random.generate Fruit_Kind fruit_kindGen] )
        ...
        Place_Fruit fruit_pos ->
            if List.member fruit_pos (model.snake).snake_body then
                ...
                let
                    nfruit = model.fruit
                    nnfruit = { nfruit | fruit_pos = fruit_pos }
                in
                ( { model | fruit=nnfruit }, cmd )
                ...
                kindd =
                    case kind of
                        ...
                        _->
                            "grape"
                nfruit = model.fruit
                nnfruit = { nfruit | fruit_type = kindd }
            in
                ( { model | fruit = nnfruit } , Cmd.none)
