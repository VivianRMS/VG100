...
checkfullLine : Int -> Int -> Grid a -> Bool
checkfullLine lineY wid grid =
    let
        ngrid = List.filter (\{ pos } -> Tuple.second pos == lineY) grid
    in
        if List.length ngrid == wid then
            True
        else
            False
...
clearLines : Int -> Int -> Grid a -> ( Grid a, Int )
clearLines wid hei grid =
    case fullLine wid grid of
        ...
        Just lineY ->
            let
                clearedGrid =
                    if (lineY /= ( hei - 1 ) ) then
                            if ( checkfullLine ( lineY+1 ) wid grid == True || checkfullLine lineY wid grid == False) then
                                List.filter (\{ pos } -> Tuple.second pos /= lineY ) grid
                            else
                                List.filter (\{ pos } -> Tuple.second pos /= lineY && Tuple.second pos /= ( lineY + 1 ) ) grid
                    else
                        List.filter (\{ pos } -> Tuple.second pos /= lineY ) grid
                ...
                cl =
                    if ( lineY == hei - 1 ) then
                        1
                    else if ( checkfullLine ( lineY+1 ) wid grid == True ) then
                        1
                    else if ( checkfullLine ( lineY ) wid grid == False ) then
                        1
                    else
                        2
                ...
                droppedAbove =
                    List.map (\c -> { c | pos = ( Tuple.first c.pos,  (Tuple.second c.pos) + cl ) }) above

                ( newGrid, lines ) =
                    clearLines wid hei (droppedAbove ++ below)
            in
            ( newGrid, lines + cl )
